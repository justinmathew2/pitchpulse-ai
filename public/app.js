...
function initTelemetryChart() {
    ...
    setInterval(() => {
        ...
        // Update top-bar entry rate display
        document.getElementById('entryRateVal').innerHTML = `${newValue} <span class="unit">/min</span>`;
    }, 10000);
}
...
function speakText(text) {
    ...
    synth.cancel(); // Cancel current speaker
...
function initMapInteractions() {
    ...
    svgNodes.forEach(node => {
        node.addEventListener('click', () => {
            ...
            // Set start or end based on context
    ...
    // Quick-seat search from Dashboard
...
function calculatePath(startId, endId) {
    ...
    // Draw route. If ADA is checked, we bypass steps and add elevator waypoint
    let pathD = '';
    let directions = [];
    let distance = 0;
    let time = 0;
    ...
    if (isAda && (startId === 'gate-d' || endId === 'block-102' || startId === 'gate-a' || endId === 'node-firstaid')) {
        // ADA routes through specialized elevator/ramp (e.g. West node coords)
        ...
        // Standard path: route along stadium concourse (curved path Q or C)
        ...
        // Generate directions based on location type
    ...
    document.getElementById('mapOverlayInfo').innerHTML = `<i data-lucide="info" class="inline-icon"></i> Route generated! Est. time: <strong>${time} min</strong>.`;
    lucide.createIcons();
}

// Sustainability State Updates
...
// Setup Event Listeners and Modal settings
function initFormsAndButtons() {
    ...
    // Sustainability form submission
...
async function sendChatMessage() {
    ...
    // Bot Typing status bubble
    const typingBubble = appendChatMessage('bot', `<span class="pulse-icon">Stadium Buddy is typing...</span>`);
...
`;
    ...
    try {
        ...
        if (appState.apiKey) {
            ...
            // Simulated Response Engine (Matches terms)
            await new Promise(r => setTimeout(r, 1200)); // typing delay
            responseText = generateMockChatResponse(chatText, lang);
        }

        typingBubble.innerHTML = responseText;
        speakText(responseText.replace(/<\/?[^>]+(>|$)/g, "")); // Speak response strip HTML
    } catch (err) {
        typingBubble.innerHTML = `<span class="text-danger"><i data-lucide="alert-triangle"></i> Error: ${err.message}</span>`;
        lucide.createIcons();
    }
}
...
function appendChatMessage(sender, htmlText) {
    ...
    msgDiv.innerHTML = `<div class="message-bubble">${htmlText}</div>`;
...
function generateMockChatResponse(query, lang) {
    ...
    // Simple response translations dictionary
    const responses = {
        en: {
...
async function submitEcoAction() {
    ...
    logBtn.innerHTML = `<span class="pulse-icon">Auditing action...</span>`;
...
`;
    ...
    try {
        ...
        if (auditResult.verified) {
            ...
            // Add carbon to dashboard
            const curCarbon = parseFloat(document.getElementById('carbonSavedVal').textContent.replace(',', ''));
            document.getElementById('carbonSavedVal').innerHTML = `${(curCarbon + auditResult.savedCo2).toFixed(1)} <span class="unit">kg</span>`;
        ...
        logBtn.innerHTML = `<i data-lucide="leaf"></i> Validate & Log Action`;
        lucide.createIcons();
    }
}
...
async function submitIncident() {
    ...
    const staff = document.getElementById('reporterStaff').value.trim();

    if (!desc) return;

    const submitBtn = document.getElementById('logIncidentBtn');
    submitBtn.disabled = true;
    submitBtn.innerHTML = `<span class="pulse-icon">Commander evaluating...</span>`;
...
`;
    ...
    try {
        ...
        submitBtn.innerHTML = `<i data-lucide="shield-alert"></i> Submit Incident to GenAI Commander`;
        lucide.createIcons();
    }
}
...
function renderIncidentLogs() {
    ...
    appState.incidents.forEach(inc => {
        ...
        item.innerHTML = `
            <div class="log-meta">
                <span class="log-location">${inc.location}</span>
                <span class="log-time">${inc.time}</span>
            </div>
            <p class="log-desc">${inc.desc}</p>
            <span class="log-tag">${inc.status === 'resolved' ? 'Resolved' : 'Active Dispatch'}</span>
        `;
...
async function handleBroadcastTranslation() {
    ...
    translateBtn.innerHTML = `<span class="pulse-icon">Translating translations...</span>`;
...
`;
    ...
    try {
        ...
        translateBtn.innerHTML = `<i data-lucide="globe"></i> GenAI Auto-Translate & Broadcast`;
        lucide.createIcons();
    }
}
